...
public class Controller implements Initializable {
    ...
    private final int roadWidth = 3;
    private final int hospitalCircleRadius = 10;
    private final int patientCircleRadius = 5;
    ...
    private final Color countryAreaColor = Color.valueOf("#cdbe98");
    ...
    @FXML
    public void openMapFileClicked() {
        ...
        for (Szpital szpital : borderHospitals) {
            borderPoints[iter++] = (double) convertPointX(szpital.getX());
            borderPoints[iter++] = (double) convertPointY(szpital.getY());
        }
        Polygon border = new Polygon();
        border.getPoints().addAll(borderPoints);
        border.setFill(countryAreaColor);
        ...
        for (Droga droga : Dane.drogi) {
            ...
            line.setStrokeWidth(roadWidth);
            map.getChildren().add(line);
        }

        for (Szpital szpital : Dane.szpitale) {
            Circle circle = new Circle(convertPointX(szpital.getX()), convertPointY(szpital.getY()), hospitalCircleRadius);
    ...
    private void setPopupWindowPosition(TextFlow textFlow, double windowPositionX, double windowPositionY) {
    ...
    @FXML
    public void openPatientFileClicked() {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setInitialDirectory(new File(System.getProperty("user.dir") + "\\data"));
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Pliki tekstowe", "*.txt"));
        File file = fileChooser.showOpenDialog(new Stage());
        if (file != null) {
            Dane.readPacjent(file.getAbsolutePath());
        }
    }

    @FXML
    private void addPatientOnClick(Event e) {
        ...
        Circle circle = new Circle(mouseEvent.getSceneX() - 34, mouseEvent.getSceneY() - 34, patientCircleRadius);
        newPatient.setNode(circle);
        patientsQueue.add(newPatient);
        circle.setFill(patientColor);
        map.getChildren().add(circle);
    }
    ...
    private void moveNextPatient() {
        ...
        if (patientsQueue.size() != 0) {
            ...
            Circle circle = new Circle(convertPointX(pacjent.getX()), convertPointY(pacjent.getY()), patientCircleRadius);
